...
export default function SeguimientosDemanda({ demandas, clientes, cargando, error, onVer }) {
  ...
  if (!pendientes.length) return <div className="rounded-2xl border border-dashed border-slate-300 neu-panel py-12 text-center"><CalendarClock className="mx-auto mb-2 text-slate-300" size={34} /><p className="font-medium text-slate-600">No hay seguimientos pendientes.</p></div>;
  return <div className="space-y-6">{grupos.map(([clave, titulo]) => { const items = pendientes.filter((d) => grupoFecha(d.fecha_seguimiento) === clave); return <section key={clave}><h2 className="mb-2 text-sm font-extrabold text-slate-700">{titulo} <span className="text-slate-400">({items.length})</span></h2>{items.length ? <div className="space-y-3">{items.map((d) => { const cliente = clientes.find((c) => Number(c.id) === Number(d.cliente_id)); const telefono = d.telefono_contacto || cliente?.celular || cliente?.telefono; return <article key={d.id} className="flex flex-col gap-3 neu rounded-2xl p-4 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-bold text-slate-800">{nombreProducto(d)}</h3><p className="mt-1 text-sm text-slate-600">{nombreContacto(d, clientes)}</p>{telefono && <p className="mt-1 flex items-center gap-1 text-xs text-slate-500"><Phone size={13} />{telefono}</p>}<p className="mt-1 text-xs text-slate-400">{diasDesde(d.fecha_registro)} días desde registro</p></div><div className="flex items-end justify-between gap-3 sm:flex-col"><div className="sm:text-right"><span className="rounded-full bg-blue-100 px-2.5 py-1 text-[11px] font-bold text-blue-800">{d.estado.replaceAll("_", " ")}</span><p className="mt-2 text-xs text-slate-500">Seguimiento: {fechaCorta(d.fecha_seguimiento)}</p></div><button onClick={() => onVer(d.id)} className="flex min-h-10 items-center gap-1 rounded-lg px-3 text-sm font-bold text-blue-700 hover:bg-blue-50"><Eye size={15} /> Ver</button></div></article>; })}</div> : <div className="rounded-xl border border-dashed border-slate-200 neu-panel px-4 py-5 text-center text-xs text-slate-400">Sin seguimientos en este grupo.</div>}</section>; })}</div>;
}

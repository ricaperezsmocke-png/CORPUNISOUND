...
function Tarjeta({ etiqueta, valor, detalle }) {
  return <div className="min-w-0 neu rounded-2xl p-4"><p className="truncate text-xs font-semibold uppercase tracking-wide text-slate-500">{etiqueta}</p><p className="mt-2 break-words text-2xl font-extrabold text-slate-900">{valor}</p>{detalle && <p className="mt-1 text-xs text-slate-500">{detalle}</p>}</div>;
}
...
export default function AnalisisDemanda({ permisos = [] }) {
  ...
  return <div className="min-w-0 space-y-5">
    <section className="neu rounded-2xl p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
        ...
        {puedeVerTodas && sucursales.length > 1 && <label className="text-xs text-slate-500">Sucursal<select value={sucursal} onChange={(e) => setSucursal(e.target.value)} className="mt-1 block min-h-10 rounded-lg border px-3 text-sm"><option value="todas">Todas</option>{sucursales.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}</select></label>}

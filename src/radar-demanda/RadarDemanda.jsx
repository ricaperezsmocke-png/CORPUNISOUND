...
import { ClipboardList, Plus, RadioTower, RefreshCw, UserRoundCheck } from "lucide-react";
...
import { cargarCatalogosRadar, listarDemandas, mensajeErrorRadar } from "./radarDemandaApi";
...
export default function RadarDemanda({ permisos = [] }) {
  ...
  const [vista, setVista] = useState(puedeRegistrar ? "registrar" : "demandas");
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      const catalogos = await cargarCatalogosRadar();
  ...
  if (!puedeVer && !puedeRegistrar) return <main className="flex min-h-full items-center justify-center bg-slate-50 px-4 py-10"><div className="max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm"><RadioTower className="mx-auto mb-3 text-slate-300" size={36} /><h1 className="text-xl font-bold text-slate-800">Acceso restringido</h1><p className="mt-2 text-sm text-slate-500">Tu rol no tiene permiso para usar Radar de Demanda.</p></div></main>;
  ...
  return <main className="min-h-full bg-slate-50 px-3 py-5 sm:px-6 sm:py-7">
    <div className="mx-auto max-w-6xl">
      ...
      <nav className="mb-5 flex flex-wrap gap-2 pb-1" aria-label="Secciones de Radar">{vistas.filter(([id]) => id === "registrar" ? puedeRegistrar : puedeVer).map(([id, etiqueta, Icono]) => <button key={id} onClick={() => setVista(id)} className={`flex min-h-11 shrink-0 items-center gap-2 rounded-xl border px-4 text-sm font-semibold ${vista === id ? "border-blue-600 bg-blue-600 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-blue-300"}`}><Icono size={17} />{etiqueta}</button>)}<button onClick={cargar} disabled={cargando} title="Actualizar" className="ml-auto flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:text-blue-700"><RefreshCw size={17} className={cargando ? "animate-spin" : ""} /></button></nav>
      ...
      {vista === "demandas" && <MisDemandas demandas={demandas} clientes={clientes} cargando={cargando} error={error} />}
      {vista === "seguimientos" && <SeguimientosDemanda demandas={demandas} clientes={clientes} cargando={cargando} error={error} />}
    </div>
  </main>;
}

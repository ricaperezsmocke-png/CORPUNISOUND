...
export default function RadarDemanda({ permisos = [] }) {
  ...
  if (!puedeVer && !puedeRegistrar && !puedeAnalizar) return <main className="flex min-h-full items-center justify-center bg-slate-50 px-4 py-10"><div className="max-w-md rounded-2xl neu p-8 text-center shadow-sm"><RadioTower className="mx-auto mb-3 text-slate-300" size={36} /><h1 className="text-xl font-bold text-slate-800">Acceso restringido</h1><p className="mt-2 text-sm text-slate-500">Tu rol no tiene permiso para usar Radar de Demanda.</p></div></main>;

  if (demandaAbierta) return <main className="min-h-full bg-background px-3 py-5 sm:px-6 sm:py-7"><div className="mx-auto max-w-6xl"><DetalleDemanda id={demandaAbierta} clientes={clientes} meta={meta} permisos={permisos || []} onVolver={() => setDemandaAbierta(null)} onActualizada={cargar} /></div></main>;

  return <main className="min-h-full bg-background px-3 py-5 sm:px-6 sm:py-7">

...
test("agregarGasto sin archivo: guarda monto/tipo, sin comprobante, suma al total y deja movimiento", async () => {
  ...
  const gasto = await agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "traslado", monto: 150 }, USUARIO, ALCANCE_TODAS, drive);
...
test("agregarGasto con archivo: sube a Drive y guarda drive_file_id/link/nombre", async () => {
  ...
  const gasto = await agregarGasto(DB, g.id, {
    forma_pago: "EFECTIVO", tipo: "reparacion", monto: 350, descripcion: "cambio de etapa",
...
test("agregarGasto rechaza tipo inválido", async () => {
  ...
  await assert.rejects(
    () => agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "viaje_a_la_luna", monto: 10 }, USUARIO, ALCANCE_TODAS, crearDriveStub()),
    /tipo de gasto/i
  );
});
...
test("agregarGasto rechaza monto <= 0 o no numérico", async () => {
  ...
  const g = crearGarantia(DB, { producto_id: 1 }, 1, USUARIO);
  const drive = crearDriveStub();
  await assert.rejects(() => agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "otro", monto: 0 }, USUARIO, ALCANCE_TODAS, drive), /monto/i);
  await assert.rejects(() => agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "otro", monto: -5 }, USUARIO, ALCANCE_TODAS, drive), /monto/i);
  await assert.rejects(() => agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "otro", monto: "abc" }, USUARIO, ALCANCE_TODAS, drive), /monto/i);
});
...
test("agregarGasto rechaza MIME no permitido", async () => {
  ...
  await assert.rejects(
    () => agregarGasto(DB, g.id, {
      forma_pago: "EFECTIVO", tipo: "traslado", monto: 10,
...
test("agregarGasto respeta el guard de alcance", async () => {
  ...
  await assert.rejects(
    () => agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "otro", monto: 10 }, USUARIO, alcanceOtra, crearDriveStub()),
    /no encontrada/i
  );
});
...
test("listarGastos respeta el alcance y devuelve solo los de esa garantía", async () => {
  ...
  const drive = crearDriveStub();
  await agregarGasto(DB, g1.id, { forma_pago: "EFECTIVO", tipo: "traslado", monto: 100 }, USUARIO, ALCANCE_TODAS, drive);
  await agregarGasto(DB, g2.id, { forma_pago: "EFECTIVO", tipo: "traslado", monto: 200 }, USUARIO, ALCANCE_TODAS, drive);
...
test("eliminarGasto borra el archivo de Drive y el registro, y deja movimiento", async () => {
  ...
  const drive = crearDriveStub();
  const gasto = await agregarGasto(DB, g.id, {
    forma_pago: "EFECTIVO", tipo: "reparacion", monto: 350,
...
test("eliminarGasto respeta el guard de alcance", async () => {
  ...
  const g = crearGarantia(DB, { producto_id: 1 }, 1, USUARIO);
  const drive = crearDriveStub();
  const gasto = await agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "otro", monto: 10 }, USUARIO, ALCANCE_TODAS, drive);
  await assert.rejects(
...
test("listarGarantias incluye total_gastos", async () => {
  ...
  const g = crearGarantia(DB, { producto_id: 1 }, 1, USUARIO);
  const drive = crearDriveStub();
  await agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "traslado", monto: 100 }, USUARIO, ALCANCE_TODAS, drive);
  await agregarGasto(DB, g.id, { forma_pago: "EFECTIVO", tipo: "reparacion", monto: 350 }, USUARIO, ALCANCE_TODAS, drive);
